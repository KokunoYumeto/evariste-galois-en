...
\GalCriticalNote{W06-PE001}{\textbf{Status: proved printed formula error;
bounded prior-notice search closed with no explicit notice found.} The second
displayed map is printed with numerator $X$ and denominator $x^n/Y$.
It is not generally equivalent to $F(x)=0$ and need not lie on the side
of $x$ opposite the first map. The full symmetric candidate
$x=\psi x=\sqrt[n]{\frac{Y}{\left(\dfrac{X}{x^n}\right)}}$ is an editorial
reconstruction, not an 1897 reading; the 1846 repetition preserves the
printed form and is not a correcting witness. The printed formula and
following claim remain unchanged in translation; downstream numerical use
requires the separately labelled reconstruction. Evidence:
\texttt{PRINTED\_AND\_SOURCE\_ERRORS.md}, W06-PE001, its cited symbolic
...
\GalCriticalNote{W06-SE001}{\textbf{Status: proved missing-hypothesis error;
bounded prior-notice search closed with no explicit notice found.} For a
positive-term polynomial $X=\sum c_jx^j$,
$nX-xX'=\sum_{j<n}(n-j)c_jx^j$ is nonnegative, but it is strictly
positive only when a coefficient below degree $n$ is positive; for
$X=x^n$ it vanishes identically and the prescribed $k$ is zero. The same
qualification is required for $Y$. The printed strict inequalities and
prescription are retained; downstream convergence claims require these
nonvanishing hypotheses. Evidence: \texttt{PRINTED\_AND\_SOURCE\_ERRORS.md},
W06-SE001, its cited exact polynomial certificate, and
