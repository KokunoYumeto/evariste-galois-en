...
\GalCriticalNote{W03-PE001}{\textbf{Status: proved printed-letter error.}
Conjugating $x=p+1/A$ gives $p+1/(-1/B)=p-B$; the two printed occurrences
of $a$ are inconsistent with the established variable $p$. Source:
PDF035/L0034. Downstream effect: the conjugation and interval argument must use
$p-B$. The bounded post-P13 search found no explicit prior notice securely
dated in the searched corpus. Evidence: \texttt{W03-PE001} in
\nolinkurl{source/critical_baseline/PRINTED_AND_SOURCE_ERRORS.md} and
\texttt{BIBLIOGRAPHIC\_PRIOR\_NOTICE\_MATRIX.csv}.}
\GalCriticalNote{W03-PE002}{\textbf{Status: proved sign error.} If $B=p+r$
with $0<r<1$, then $p-B=-r$, so $-1<p-B<0$. The printed interval “between
$0$ and $1$” has the wrong sign; it should be between $0$ and $-1$.
Source: PDF035/L0034. Downstream effect: the subsequent converse argument
already requires the corrected interval $-1<p-B<0$. The bounded post-P13
search found no explicit prior notice securely dated in the searched corpus. Evidence:
\texttt{W03-PE002} in
\nolinkurl{source/critical_baseline/PRINTED_AND_SOURCE_ERRORS.md} and
\texttt{BIBLIOGRAPHIC\_PRIOR\_NOTICE\_MATRIX.csv}.}
...
\GalCfracCorrect
\]
\GalCriticalNote{W03-PE003}{\textbf{Status: proved formula error.} The second
root is $3-A$, not the printed staircase expression $3-1/A$. Direct
substitution gives zero residual for $3-A$, whereas the printed value is not
a root of $3x^{2}-16x+18=0$. Source: PDF037/L0036. Downstream effect: the
terminal continued fraction is consistent with the repaired root $3-A$; the
printed intermediate display is not. The bounded post-P13 search found no
