...
\GalCriticalNote{W04-SE001}{The quotient needs a nonzero denominator}{The
quotient is defined only when $f(x+h)\ne f(x)$. If $f$ is identically zero,
for example, the denominator vanishes for every $x$ and $h$. Adding this
condition repairs the domain of the quotient, but not the later steps involving
an extremum, an inverse function, or the existence of a derivative.}

\GalCriticalNote{W04-SE004}{The extremum step requires regularity}{A
discontinuous function can take equal endpoint values, be nonconstant, and
still attain no relevant interior extremum. Continuity on the closed interval,
or another condition that guarantees attainment, is required. Even with that
repair, the later inverse inference and corollary need separate justification.}

\GalCriticalNote{W04-SE003}{The inverse step requires injectivity}{From
$k=\psi(P)$ one cannot infer a single-valued $P=\varphi(k)$ unless $\psi$ is
injective or an inverse branch has been specified. A noninjective $\psi$ with
$\psi(0)=\psi(1)=0$ is an immediate counterexample. This added condition
repairs only the inverse step, not the theorem's other missing hypotheses.}

\GalCriticalNote{W04-SE002}{The claimed limit need not exist}{Take
$F(t)=|t|$, $f(t)=t$, and $x=0$. The quotient becomes $|h|/h$, whose
right-hand limit is $1$ and left-hand limit is $-1$. No local change of wording
can make the printed universal claim true; a valid theorem needs explicit
domain and regularity hypotheses, and the stated corollary does not establish
the universal existence of derivatives.}
