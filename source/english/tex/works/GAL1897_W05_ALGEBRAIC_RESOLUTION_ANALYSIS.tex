...
\GalCriticalNote{W05-SE001}{\textbf{Status: proved source error; bounded prior-notice search closed with no explicit notice found.}
The finite exception list is not a complete modern classification. A certified
counterexample has degree $16$: on $V=\mathbf F_2^4$, an irreducible solvable
complement $(C_3\times C_3)\rtimes C_2$ of order $18$ gives a primitive affine
group of order $288$. It is not one-dimensional semilinear, since
$18\nmid|\Gamma L(1,16)|=60$. The correct general framework is that a finite
solvable primitive group is affine, with regular elementary-abelian socle and
faithful irreducible point stabilizer; one-dimensional semilinearity is an
additional condition. The historical sentence remains unchanged, and the
downstream degree-$9$/$25$-only claim is rejected in \texttt{POST-P13-A005}.
Evidence: W05-SE001 and POST-P13-A005 in the post-P13 catalogue and mathematical
checks; \texttt{BIBLIOGRAPHIC\_PRIOR\_NOTICE\_MATRIX.csv}.}
...
\GalCriticalNote{W05-SE002}{\textbf{Status: proved source error with an explicit
1846 editorial notice.}
The main text asserts a universal impossibility in higher degrees, but the
following Liouville note records reductions for $p=7$ and $p=11$ as well as
$p=5$. Exact subgroup calculations independently verify the $p=7$ and $p=11$
exceptions. The erroneous sentence and its historical correction remain
separate. For $p=11$, the repaired six-pair matching has stabilizer order $60$
and index $11$ in $PSL(2,11)$; see W08-PE001. Downstream statements may use the
corrected list only as explicitly labelled critical reconstruction. Liouville's printed 1846 footnote is the
earliest secure notice recorded by the post-P13 search. Evidence:
\texttt{PRINTED\_AND\_SOURCE\_ERRORS.md}, W05-SE002, its cited group-theoretic
certificate, and \texttt{BIBLIOGRAPHIC\_PRIOR\_NOTICE\_MATRIX.csv}.}
